<template>
  <div>
    ...
    <div v-if="!useConnectionString" class="mt-3">
      <p>And then download and replace the <code>config.json</code> in the <em>ghmattimysql</em> folder with the one
    ...
    <div v-if="useConnectionString" class="mt-3">
      <p>And then make sure you have deleted the <code>config.json</code> in the <em>ghmattimysql</em> folder.</p>
    </div>

    <p class="mt-2 p-2 font-weight-bold">Congratulations, you have successfully setup <em>ghmattimysql</em> to work with the FiveM server.</p>
  </div>  
</template>
